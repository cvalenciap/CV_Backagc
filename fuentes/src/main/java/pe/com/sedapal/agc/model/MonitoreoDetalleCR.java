...
public class MonitoreoDetalleCR {
		...
		if (respuestaConsulta.get("C_OUT") != null) {
			...
			if (!listaMaps.isEmpty()) {
				...
				for (Map<String, Object> map : listaMaps) {
					...
					item.setCodigoActividad(CastUtil.leerValorMapStringOriginCase(map, "CACT"));
					...
					item.setLectura(CastUtil.leerValorMapStringOriginCase(map, "LECT"));
//					item.setLecturaCierre(CastUtil.leerValorMapStringOriginCase(map, "LECT"));
					...
					//zona.setDetalle("SIN ASIGNAR");
					zona.setDetalle(CastUtil.leerValorMapString(map, "DESC_ZONA"));
					item.setZona(zona);
